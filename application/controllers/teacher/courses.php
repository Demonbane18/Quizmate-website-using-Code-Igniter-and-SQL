...
HTML;
	...
	$('#addPaper').click(function(e) {
		...
		dialog.modal('show');
		dialog.find('.modal-title').html('<i class="fa fa-plus"></i> Create quiz set');
...
HTML;
	...
	public function add()
	{
		...
		if ($this->input->post('submit'))
		{
			...
			$this->form_validation->set_message('required', 'You must select/fill %s');
			if ($this->form_validation->run())
			{
				...
				if ($this->courses->addCourse($courseData))
				{
					...
					$this->session->set_flashdata('msg_error',
						'Something went wrong cannot add courses');
					redirect($this->role.'/courses');
				}
			}
			else
	...
	function addpaper($courseId='')
	{
		...
		if ($method == "add")
		{
			...
			$this->session->set_flashdata('msg_info',
						'Add quiz set '.$addPapaer['name'].' successfully');
	...
	function editpaper($paperid,$courseId)
	{
		...
		$this->session->set_flashdata('msg_info',
					'Edited quiz set '.$editPaper['name'].' Successfully');
	...
	function addpart($courseId='')
	{
		...
		$this->session->set_flashdata('msg_info',
					'Add chapter '.$partData['title'].' successfully');
				redirect($this->role.'/courses/view/'.$courseId.'#papers');
	}
