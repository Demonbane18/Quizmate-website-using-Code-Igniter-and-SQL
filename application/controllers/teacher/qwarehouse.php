	...
	$("#chapterAdd").on("click", function(e) {
		...
		.fail(function(jqxhr, textStatus, error) {
			...
			var jbox = new jBox(\'Modal\', {
				...
				title: \'Error\',
				overlay: true,
				content: \'Unable to add chapter. Please check if that it is correct.\',
			});
			jbox.open();
	...
	function delFunc(delElem) {
		...
		.fail(function(jqxhr, textStatus, error) {
			...
			var jbox = new jBox(\'Modal\', {
				...
				title: \'Error\',
				overlay: true,
				content: \'Unable to add chapter. Please check that if it is correct.\',
			});
			jbox.open();
	...
	$("#addQuestion").click(function(e) {
		...
		if (chapter_id == undefined)
		{
			var jbox = new jBox(\'Modal\', {
				...
				title: \'Error\',
				overlay: true,
				content: "Must choose Chapter before"
			});
			jbox.open();
			btnAddState("normal");
			return false;
	...
	function callbackjson()
	{
		...
		switch ($arg_list[0]) {
			...
			case 'addQuestion':
				...
				// Self-Validate Input Data
				$msgerr = "Cannot be addded due to error<br><ul>";
				$errObj = array();

				if ($chapter_id == "" || !is_numeric($chapter_id))
				{
					$msgerr .= "<li>Data not found or Chapter ID is incorrect</li>";
				}
				...
				{
					$msgerr .= "<li>Data not found or Question type is incorrect</li>";
					$errObj['qtype'] = 1;
				}
				if (trim(strip_tags($question)) == "")
				{
					$msgerr .= "<li>Must have questions for quiz takers</li>";
					$errObj['question'] = 1;
				}
				if ($answer == "")
				{
					$msgerr .= "<li>There must have a correct answer</li>";
					$errObj['correct'] = 1;
				}
				...
				switch ($question_type) {
					case 'choice':
						...
						{
							$msgerr .= "<li>Objective : Must have at least 2 option</li>";
							$errObj['choices'] = 1;
						}
						if ($answer != "" || is_numeric($answer))
						{
							...
							{
								$msgerr .= "<li>Objective : Didn't answer!</li>";
								$errObj['choices'] = 1;
						...
						{
							$msgerr .= "<li>Objective : Must choose the correct answer</li>";
							$errObj['choices'] = 1;
						}
						break;
					case 'numeric':
						...
						{
							$msgerr .= "<li>Objective : Correct answer must be specified using numbers</li>";
							$errObj['correct'] = 1;
						}
						break;
					case 'boolean':
						...
						{
							$msgerr .= "<li>True/False : Choose the correct answer, true or false</li>";
							$errObj['correct'] = 1;
						}
						break;

					default:
						$msgerr .= "<li> Question type is incorrect</li>";
						break;
				}

				$msgerr .= "</ul>";

				if ($msgerr != "Cannot be added due to the following error<br><ul></ul>")
				{
					echo json_encode(array(
						'error' => $msgerr,
						'objects' => $errObj
				...
				switch ($question_type) {
					...
					case 'matching':
						throw new Exception("Case Not implemented yet", 1);
						break;
		...
		else
		{
			...
			else
			{
				...
				if (!empty($data['subjectInfo']))
				{
					...
					$data['pagetitle'] = "Organize quiz";
					$data['pagesubtitle'] = "Subjects ".$data['subjectInfo']['code']." ".$data['subjectInfo']['name'];
