...
if ($('#subjectid').val() == '')
{
	$('#courseDesc').html('<h4>...Please select a subject...<h4>');
}
...
HTML;
		...
		$this->papers = <<<HTML
	// Open, create quiz sets
	$('#addPaper').click(function(e) {
		var dialog = $('#modaladdpaper');
		dialog.modal('show');
		dialog.find('.modal-title').html('<i class="fa fa-plus"></i> Creat quiz set');
		dialog.find("button[type='submit']").html('<i class="fa fa-plus"></i> Create');
	...
	$('.paper-list').delegate('.list-group-item .optionlinks a.edit', 'click', function(e) {
		...
		var dialog = $('#modaladdpaper');
		dialog.modal('show');
		dialog.find('.modal-title').html('<i class="fa fa-edit"></i> Edit quiz set');
		dialog.find("button[type='submit']").html('<i class="fa fa-save"></i> Record');
...
HTML;
	...
	public function view($courseId='')
	{
		...
		else
		{
			...
			else
			{
				...
				$data['pagetitle'] = "Quiz Information";
	...
	public function add()
	{
		...
		$data['pagetitle'] = "Open for new subjects";
		$data['pagesubtitle'] = "";
		...
		if ($this->input->post('submit'))
		{
			$this->form_validation->set_rules('subjectid', 'subject code', 'required');
			...
			if ($this->form_validation->run())
			{
				...
				if ($this->courses->addCourse($courseData))
				{
					...
					$this->session->set_flashdata('msg_error',
						'Something went wrong unable to add subjects');
					redirect($this->role.'/courses');
				}
			}
			else
	...
	public function edit($courseId)
	{
		$this->form_validation->set_rules('subjectid', 'Subject code', 'required');
		$this->form_validation->set_rules('year', 'year', 'required');
		$this->form_validation->set_message('required', 'You must selectt/fill %s');
		if ($this->form_validation->run())
		{
			# Form check completed
			$courseData['course_id'] = $courseId;
			...
			if ($updateTeasRes != 0) {
				$this->session->set_flashdata('msg_error',
					'Something went wrong in adding teachers '.$updateTeasRes);
				redirect($this->role.'/courses');
			}
			...
			if ($this->courses->updateCourse($courseData, $courseId))
			{
				...
				# Failed
				$this->session->set_flashdata('msg_error',
					'Something went wrong cannot be updated');
				redirect($this->role.'/courses');
			}
		}
		else
	...
	function addpaper($courseId='')
	{
		...
		if ($method == "add")
		{
			...
			$this->session->set_flashdata('msg_info',
						'Add quiz set '.$addPapaer['name'].' already');
	...
	function editpaper($paperid,$courseId)
	{
		...
		$this->session->set_flashdata('msg_info',
					'Adjust exam set '.$editPaper['name'].' already');
	...
	function addpart($courseId='')
	{
		...
		$this->session->set_flashdata('msg_info',
					'Add course '.$partData['title'].' already');
				redirect($this->role.'/courses/view/'.$courseId.'#papers');
	}
	...
	function editpart($partId='')
	{
		...
		$data['pagetitle'] = "Quiz set editor";
	...
	var resortOrder = function() {
		$("#selectedquestions .box").each(function(index,elem) {
			...
			$(elem).find(".question-labelno").text("Clause");
		});
